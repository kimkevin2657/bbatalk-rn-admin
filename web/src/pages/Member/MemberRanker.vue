  ...
  <div>
    ...
    <div class="q-pa-md">
      ...
      <div class="row justify-between">
        <div class="col-4 text-h5">랭커 회원 현황 - 1위부터 미친듯이 쭈욱 나오게 해줘</div>
        <q-page-sticky position="top-right" :offset="[16, 16]">
          <div class="col-3 q-gutter-sm">
            <q-btn color="brown-5" label="수강 등록" @click="onClickLessonRegistBtn" />
            <q-btn color="brown-5" label="옵션 등록" @click="onClickOptionRegistBtn" />
            <q-btn color="brown-5" label="회원 등록" @click="onClickMemberRegistBtn" />
          </div>
        </q-page-sticky>
      </div>
      <!-- 검색 조건 그리드  -->
      <div style="padding: 15px 0px; margin: 0px 0px">
        <div class="row q-col-gutter-none" style="background: rgba(255,255,255,1); border: 1px solid rgba(122,122,124,.2)">
          <!-- 담당자 구분 -->
          <div class="col-3 condition-box" style="min-width: 200px;">
            <q-select
              dense
              filled
              v-model="manager"
          ...
          <div class="col-2 q-pa-md form-date">
            <q-input dense filled v-model="startDate" mask="date" :rules="['date']" label="시작 일자">
          ...
          <div class="col-2 q-pa-md form-date">
            <q-input dense filled v-model="endDate" mask="date" :rules="['date']" label="종료 일자">
              <template v-slot:append>
                <q-icon name="event" class="cursor-pointer">
                  <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
      ...
      >
        <template v-slot:body="props">
          ...
          <q-td key="name" :props="props">
            <q-btn flat style="color: #027BE3">{{ props.row.name }}</q-btn>
          </q-td>
          ...
          <q-td key="member_type" :props="props">{{ props.row.member_type }}</q-td>
          <q-td key="membership_type" :props="props">
            <q-badge square color="amber">{{ props.row.membership_type }}</q-badge>
            <div class="table-membership">
              <q-btn flat style="min-width: 200px; color: #027BE3; padding-left: 0px 0px">{{ props.row.membership_date }}</q-btn>
            </div>
            </q-td>
          <q-td key="lesson_type" :props="props">
            <q-badge square color="purple">{{ props.row.lesson_type }}</q-badge>
            <div class="table-membership">
              <q-btn flat style="min-width: 200px; color: #027BE3;">{{ props.row.lesson_date }}</q-btn>
            </div>
          </q-td>
          <q-td key="option_info" :props="props">{{ props.row.option_info }}</q-td>
          <q-td key="gym_experience_info" :props="props">{{ props.row.gym_experience_info }}</q-td>
          <q-td key="lesson_experience_type" :props="props">
            <q-badge square color="purple">{{ props.row.lesson_experience_type }}</q-badge>
            <div class="table-membership">
...
export default {
  ...
  methods: {
    ...mapActions('userSettings', [
      'reqGetUserDataList', // 모든 회원 리스트 가져오기 요청
      'reqAddUser' // 회원 추가 요청
    ]),
