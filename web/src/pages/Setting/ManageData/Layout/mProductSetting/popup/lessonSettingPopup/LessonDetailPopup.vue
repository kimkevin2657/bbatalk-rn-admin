<template>
  ...
  <div class="row">
    ...
    <q-input
      ref="defineLessonCnt"
      name="defineLessonCnt"
      v-model.number="selectedData.totCnt"
      type="number"
      label="횟수"
      suffix="회"
      data-vv-scope="regdetail"
      v-validate="'required'"
      :error="errors.has('regdetail.defineLessonCnt')"
      :error-message="getValidateMessage('defineLessonCnt')"
      autofocus
      dense
      class="col-12"
...
export default {
  ...
  computed: {
    ...
    defPrice () {
      return this.selectedData.totCnt * this.lessonDefineData.defPrice
    },
    priceData () {
      return (this.selectedData.totCnt * this.lessonDefineData.defPrice) - this.selectedData.discount
    }
  },
  methods: {
    async onAdd (isContinue) {
      ...
      // 중복확인
      const duplIndex = _.findIndex(this.lessonDefineData.product, o => {
        return o.totCnt === this.selectedData.totCnt && o.month === this.selectedData.month
      })
      ...
      let tempDataList = _.cloneDeep(this.lessonDefineData.product)
      tempDataList.push(row)
      this.lessonDefineData.product = _.sortBy(tempDataList, ['totCnt', 'month'])
    ...
    async onEdit () {
      ...
      // 중복확인
      const duplIndex = _.findIndex(this.lessonDefineData.product, o => {
        return o.totCnt === this.selectedData.totCnt && o.month === this.selectedData.month && o.id !== this.selectedData.id
      })
      if (duplIndex !== -1) return this.$notify.error('중복되는 설정이 있습니다.')
