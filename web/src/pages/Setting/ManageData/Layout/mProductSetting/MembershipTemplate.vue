<template>
  <div>
    <q-table
      ...
      selection="multiple"
      separator="cell"
      no-data-label="데이터를 찾지 못하였습니다. (회원권을 추가하세요.)"
    ...
    >
      ...
      <template #top>
        <div class="text-h6 col-12">회원권</div>
    ...
    <membership-setting-popup ref="regPopup" />
  </div>
</template>
...
// Popup
import MembershipSettingPopup from './popup/MembershipSettingPopup'
...
export default {
  ...
  components: {
    MembershipSettingPopup,
    EditTemplate
  },
  mixins: [TableMixin],
  mounted () {
    this.reqGetDefineDataList({ type: 'membership' })
  },
  data () {
    return {
      rowsSelected: [],
      filter: '',
      columns: [
        { name: 'name', align: 'left', label: '회원권 명', field: 'name', sortable: true },
