<template>
  <div>
    <q-table
      :data="marketingPathDefineDataList"
      :columns="columns"
      ...
      selection="multiple"
      separator="vertical"
      no-data-label="데이터를 찾지 못하였습니다. (인터넷, 추천 등을 추가하세요.)"
    ...
    <marketing-path-popup ref="regPopup" />
  </div>
</template>
...
// Popup
import MarketingPathPopup from './popup/MarketingPathPopup'
import EditTemplate from '../../../../../components/template/EditTemplate'
// Mixin
import TableMixin from '../../../../../mixins/tableMixin'
export default {
  name: 'MarketingPathTemplate',
  components: {
    MarketingPathPopup,
    EditTemplate
  },
  ...
  data () {
    return {
      ...
      filter: '',
      columns: [
        { name: 'name', align: 'left', label: '마케팅 경로 명', field: 'name', sortable: true },
  ...
  methods: {
    ...
    async onRemove () {
      let reqQuery = { type: 'marketingpath' }
