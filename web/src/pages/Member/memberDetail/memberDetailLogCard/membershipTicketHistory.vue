  ...
  <div>
    <q-table
      ...
      @row-dblclick="(evt, row) => {
        $refs.membershipRegistPopup.show({ticketInfo: row})
        rowsSelected = [row]
      }"
      dense
      flat
...
export default {
  ...
  async mounted () {
    // 회원권 항목
    await this.reqGetTicketDataList({ type: 'membership', memberid: this.memberData._id })
  },
  methods: {
    ...mapActions('ticket', [
