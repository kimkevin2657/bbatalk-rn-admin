<template>
  <div>
    <q-table
      :data="membershipDefineData.product"
      :columns="columns"
    ...
    <membership-detail-popup ref="regPopup" />
  </div>
</template>
  ...
  methods: {
    ...
    async onRemove () {
      if (_.size(this.rowsSelected) === 1) {
        ...
        const index = _.findIndex(this.membershipDefineData.product, o => {
          return o.id === id
        })
        this.membershipDefineData.product.splice(index, 1)
      } else {
        _.forEach(this.rowsSelected, o => {
          const index = _.findIndex(this.membershipDefineData.product, item => {
            return item.id === o.id
          })
          this.membershipDefineData.product.splice(index, 1)
        })
