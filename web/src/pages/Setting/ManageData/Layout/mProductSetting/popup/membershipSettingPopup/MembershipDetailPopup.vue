...
export default {
  ...
  computed: {
    ...
    defPrice () {
      return this.selectedData.month * this.membershipDefineData.defPrice
    },
    priceData () {
      return (this.selectedData.month * this.membershipDefineData.defPrice) - this.selectedData.discount
    }
  },
  methods: {
    async onAdd (isContinue) {
      ...
      // 중복확인
      const duplIndex = _.findIndex(this.membershipDefineData.product, o => {
      ...
      let tempDataList = _.cloneDeep(this.membershipDefineData.product)
      tempDataList.push(row)
      this.membershipDefineData.product = _.sortBy(tempDataList, ['month'])
