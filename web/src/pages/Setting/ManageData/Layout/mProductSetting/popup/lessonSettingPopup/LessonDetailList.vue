<template>
  <div>
    ...
    <lesson-detail-popup ref="regPopup" />
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      rowsSelected: [],
      columns: [
        { name: 'totCnt', align: 'right', label: '횟수', field: 'totCnt', sortable: true },
  ...
  methods: {
    ...
    async onRemove () {
      if (_.size(this.rowsSelected) === 1) {
        ...
        const index = _.findIndex(this.lessonDefineData.product, o => {
          return o.id === id
        })
        this.lessonDefineData.product.splice(index, 1)
      } else {
        _.forEach(this.rowsSelected, o => {
          const index = _.findIndex(this.lessonDefineData.product, item => {
            return item.id === o.id
          })
          this.lessonDefineData.product.splice(index, 1)
        })
